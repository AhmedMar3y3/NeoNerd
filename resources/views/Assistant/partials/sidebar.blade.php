...
<nav class="sidebar sidebar-offcanvas" id="sidebar" style="background: #0B1120">
        ...
        <li class="nav-item dropdown me-5" style=" list-style-type: none;">
            <a class="nav-link " id="profileDropdown" href="#" data-bs-toggle="dropdown">
                <div class="navbar-profile d-flex gap-2" style="color: white">
                    ...
                    <p class="mb-0 d-none d-sm-block navbar-profile-name">{{ Auth::guard('assistant')->user()->name }}</p>
                    <i class="mdi mdi-menu-down d-none d-sm-block"></i>
                    <img class="img-xs rounded-circle" 
                            src="{{ Auth::guard('assistant')->user()->image ? asset(Auth::guard('assistant')->user()->image) : asset('assets/images/dashboard/avatar.png') }}" 
                            alt="">
                </div>
            </a>
            <div class="dropdown-menu dropdown-menu-end navbar-dropdown preview-list" aria-labelledby="profileDropdown"
                ...
                <form id="logout-form" action="{{ route('assistant.logout') }}" method="POST" style="display: none;">
                    @csrf
                </form>
            </div>
        </li>
    ...
    <ul class="nav">
            ...
            <div class="profile-desc">
                <div class="profile-pic d-flex align-items-center justify-content-end gap-2 w-100">
                    ...
                    <div class="profile-name">
                        <h5 class="mb-0 font-weight-normal">{{ Auth::guard('assistant')->user()->name }}</h5>
                        <span>online</span>
                    </div>
        ...
        <li class="nav-item menu-items my-1">
            <a class="nav-link d-flex gap-2" href="{{ route('assistant.dashboard') }}">
         ...
         <li class="nav-item menu-items">
            <a class="nav-link d-flex d-block w-100 justify-content-end" href="{{ route('assistant.subscriptions.index') }}">
