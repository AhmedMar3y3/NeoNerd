...
@section('main')
<div class="lesson-edit-container" dir="rtl">
    <!-- Page Header -->
    <div class="page-header">
        <h1 class="page-title">تعديل الدرس</h1>
        <p class="page-subtitle">تحديث معلومات الدرس</p>
    ...
    <div class="lesson-form-card">
        <form action="{{ route('doctor.courses.units.lessons.update', [$course->id, $unit->id, $lesson->id]) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <div class="duration-input">
                        @php
                            ...
                            $minutes = $durationParts[0] ?? '00';
                            $seconds = $durationParts[1] ?? '00';
                        @endphp
                        <input type="number" 
                               id="duration_minutes" 
                               ...
                               min="0"
                               max="99"
                               value="{{ old('duration_minutes', $minutes) }}"
                               required>
                        ...
                        <input type="number" 
                               ...
                               min="0"
                               max="59"
                               value="{{ old('duration_seconds', $seconds) }}"
                               required>
                        <span class="duration-label">دقيقة:ثانية</span>
                    </div>
                    <input type="hidden" id="duration" name="duration" value="{{ old('duration', $lesson->duration) }}">
                    @error('duration')
                        <div class="form-error">
                            <i class="fas fa-exclamation-circle"></i>
                            {{ $message }}
